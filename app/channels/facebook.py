...
def send_fb_media_message_detailed(
    recipient_id: str,
    media_type: str,
    media_url: str,
    page_token: str = None,
    ...
    """
    Delivers a media attachment (image, video, audio) with atomic database claim idempotency.
    Returns (success: bool, meta_message_id: str).
    """
    token = page_token or get_fb_token(page_id)
    ...
    if not can_send:
        ...
        if existing_status == "SENT":
            ...
            return True, claim_record.get("meta_message_id", "")
...
def send_fb_media_message(
    ...
    """Delivers a media attachment (image, video, audio) via Facebook Messenger."""
    ok, _ = send_fb_media_message_detailed(
        recipient_id=recipient_id,
        media_type=media_type,
        media_url=media_url,
...
async def process_facebook_batch(batch: PendingBatch):
    ...
    is_voice_turn = bool(audios) or ("ভয়েস" in (combined_text or "")) or ("ভয়েস" in (combined_text or "")) or not (combined_text or "").strip()
    ...
    if media_sequence and isinstance(media_sequence, list) and len(media_sequence) > 0:
        for item in media_sequence:
            ...
            if item_type == "images":
                ...
                for img_path in item.get("urls", []):
                    ...
                    ok_img, sent_msg_id = send_fb_media_message_detailed(
                    ...
                    record_conversation_message(
                        ...
                        page_id=page_id, workspace_id=workspace_id, direction="OUTBOUND", sender_role="AI",
                        external_message_id=sent_msg_id
                    )
                    await asyncio.sleep(0.15)
            elif item_type == "text":
                text_content = item.get("text", "")
                if text_content:
                    send_fb_text_message(sender_id, text_content, page_token=page_token, page_id=page_id)
                    record_conversation_message(
                        "facebook", sender_id, customer_name, "bot", text_content,
                        page_id=page_id, workspace_id=workspace_id, direction="OUTBOUND", sender_role="AI"
                    )
                ...
                if v_url:
                    ...
                    record_conversation_message(
                        "facebook", sender_id, customer_name, "bot", "[Voice Note]", v_url,
                        page_id=page_id, workspace_id=workspace_id, direction="OUTBOUND", sender_role="AI"
                    )
                    await asyncio.sleep(0.2)
    else:
        ...
        for img_path in matched_images:
            if not img_path:
                continue
            full_img_url = img_path if img_path.startswith("http") else f"{base_server_url}{img_path}"
            img_sent, sent_msg_id = send_fb_media_message_detailed(
                recipient_id=sender_id,
                media_type="image",
                media_url=img_path,
                page_token=page_token,
        ...
        if not reply_text and fb_img_sent_count > 0 and sender_id:
            ...
            if "changes" in entry:
                ...
                for change in entry.get("changes", []):
                    ...
                    if auto_comment and comment_id:
                        if comment_intent == "gratitude":
                            # Case A: Gratitude / Compliment / Prayer (e.g. শুকরিয়া, ধন্যবাদ, মাশাল্লাহ, অনেক সুন্দর)
                            # Reply with warm, respectful appreciation without asking to check inbox, and NEVER quote MOQ or order constraints!
                            clean_cmt = comment_text.lower()
                            if "মাশাআল্লাহ" in clean_cmt or "মাশাল্লাহ" in clean_cmt or "masha" in clean_cmt:
                                public_reply_text = f"আলহামদুলিল্লাহ, অসংখ্য ধন্যবাদ {user_name} {honorific}! আপনার সুন্দর দোয়ার জন্য জাযাকাল্লাহু খাইরান। 🤲🥰"
                            elif "জাযাকাল্লাহ" in clean_cmt or "জাজাকাল্লাহ" in clean_cmt or "jazak" in clean_cmt:
                                public_reply_text = f"ওয়া ইয়্যাকুম {user_name} {honorific}! অসংখ্য ধন্যবাদ ও শুভকামনা আপনার জন্য। ❤️"
                            elif "ধন্যবাদ" in clean_cmt or "thanks" in clean_cmt or "thank you" in clean_cmt or "শুক্রিয়া" in clean_cmt or "শুকরিয়া" in clean_cmt:
                                public_reply_text = f"আপনাকেও অসংখ্য ধন্যবাদ {user_name} {honorific}! আপনার সুন্দর মতামতের জন্য আমরা আন্তরিকভাবে কৃতজ্ঞ। 🥰"
                            else:
                                template = get_setting("gratitude_comment_template", f"আলহামদুলিল্লাহ, অসংখ্য ধন্যবাদ {user_name} {honorific}! আপনার সুন্দর মন্তব্যের জন্য আন্তরিক কৃতজ্ঞতা ও ভালোবাসা রইল। 🥰")
                                public_reply_text = template.replace("{name}", user_name).replace("{honorific}", honorific)
                    ...
                    if send_private and comment_id and comment_intent == "inquiry":
                        # Send directly what the customer asked
                        inbox_res = await process_customer_message(
                            message_text=comment_text or "আইডি কার্ড ও সার্ভিসের বিস্তারিত দাম ও বিবরণ দিন",
                            channel="facebook",
